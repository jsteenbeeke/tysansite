...
@Component
public class SenateElectionWithOnlyASingleCandidatePopulator implements IEntityPopulator {
	public static final String KEY = "senate-election-started-with-only-single-candidate";
	@Autowired
	...
	public void createEntities() {
		...
		userService.setUserImportData(user3.getId(), Rank.JUNIOR_MEMBER, i.getEpochSecond());
		...
		e.setSeats(2);
		e.setStart(Date.from(LocalDate.now().minusDays(8L).atStartOfDay().toInstant(ZoneOffset.UTC)));

		senateElectionDAO.save(e);

		e.getCandidates().add(user2);

		senateElectionDAO.update(e);

		senateElectionDAO.flush();
