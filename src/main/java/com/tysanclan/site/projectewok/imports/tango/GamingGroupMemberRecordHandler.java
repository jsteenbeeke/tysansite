 ...
 */
public class GamingGroupMemberRecordHandler implements
        RecordHandler {
	@SpringBean
	...
	public boolean handle(String[] data,
		...
		// GGM userkey groupkey isLeader
		long userKey = Long.parseLong(data[1]), groupKey = Long
		        .parseLong(data[2]);
		boolean isLeader = data[3].equals(VALUE_TRUE);

		if (callback.hasImportedObject(userKey, User.class)
		        && callback.hasImportedObject(groupKey,
		                GamingGroup.class)) {
			User user = callback.getImportedObject(userKey,
			        User.class);
			GamingGroup group = callback.getImportedObject(
			        groupKey, GamingGroup.class);
			if (groupService.addUserToGroup(user, group)) {
				if (isLeader) {
					return groupService.setGroupLeader(
					        user, group);
				}
