...
public class CaretakerFinancePage extends AbstractMemberPage {
	...
	public CaretakerFinancePage(User user) {
		...
		add(new ListView<Expense>("expenses", ModelMaker
				.wrap(expenseDAO.findByFilter(efilter).toJavaList())) {
			...
			protected void populateItem(ListItem<Expense> item) {
				...
				item.add(new Label("regularity",
						expense.getPeriod().toString()));
				...
				if (expense.getEnd() != null) {
					...
					item.add(new IconLink.Builder(
							...
							new DefaultClickResponder<Expense>(
								...
								public void onClick() {
									financeService
											.setExpenseEnded(getModelObject(),
													new Date());
				...
				item.add(new IconLink.Builder("images/icons/money.png",
						new DefaultClickResponder<Expense>(
							...
							public void onClick() {
								setResponsePage(
										new EditExpensePage(getModelObject()));
							}
		...
		add(new ListView<Donation>("donations", ModelMaker
				.wrap(donationDAO.findByFilter(filter).toJavaList())) {
			private static final long serialVersionUID = 1L;
			...
			protected void populateItem(ListItem<Donation> item) {
				...
				if (donation.getDonator() == null) {
					...
					item.add(
							new MemberListItem("donor", donation.getDonator()));
				}
				...
				item.add(new IconLink.Builder("images/icons/cross.png",
						new DefaultClickResponder<Donation>(
								ModelMaker.wrap(donation)) {
							private static final long serialVersionUID = 1L;
							...
							public void onClick() {
								...
								setResponsePage(
										new CaretakerFinancePage(getUser()));
							}
						}).newInstance("delete"));
		...
		add(new Label("exhaustion", t.getExhaustionPoint() == null ?
				"for the foreseeable future (at least 3 years)" :
	...
	private Form<Expense> createExpenseForm() {
		Form<Expense> expenseForm = new Form<Expense>("addexpenseform") {
			...
			protected void onSubmit() {
				DropDownChoice<String> regularityDropDown = (DropDownChoice<String>) get(
						"regularity");
				DropDownChoice<Integer> currencyMajorDropDown = (DropDownChoice<Integer>) get(
						"currencymajor");
				DropDownChoice<Integer> currencyMinorDropDown = (DropDownChoice<Integer>) get(
						"currencyminor");
				TextField<String> expenseNameField = (TextField<String>) get(
						"expensename");
				DatePickerPanel startDatePicker = (DatePickerPanel) get(
						"startdate");
				...
				Expense expense = financeService
						.createExpense(expenseName, amount, period, startDate);
		...
		Form<Donation> donationForm = new Form<Donation>("donationform") {
			...
			protected void onSubmit() {
				DropDownChoice<User> userSelect = (DropDownChoice<User>) get(
						"userselect");
				DropDownChoice<Integer> currencyMajor = (DropDownChoice<Integer>) get(
						"currencymajor");
				DropDownChoice<Integer> currencyMinor = (DropDownChoice<Integer>) get(
						"currencyminor");
				DatePickerPanel donationDatePicker = (DatePickerPanel) get(
						"donationdate");

				BigDecimal amount = new BigDecimal(
						100 * currencyMajor.getModelObject() + currencyMinor
								.getModelObject()).divide(new BigDecimal(100));

				Donation donation = financeService
						.createDonation(userSelect.getModelObject(), amount,
