...
public class PastElectionsPage extends AbstractMemberPage
{
	...
	public PastElectionsPage()
	{
		...
		accordion.add(new ListView<Election>("elections", ModelMaker.wrap(elections))
		{
			...
			protected void populateItem(ListItem<Election> item)
			{
				...
				Collections.sort(votes, new Comparator<CompoundVote>()
				{
					...
					{
						return o1.getCaster().getUsername()
				...
				item.add(new WebMarkupContainer("candidatesheader")
					.add(new SimpleAttributeModifier("colspan", Integer.toString(election
						.getCandidates().size()))));
				...
				item.add(new ListView<CompoundVote>("votes", ModelMaker.wrap(votes))
				{
					...
					protected void populateItem(ListItem<CompoundVote> innerItem)
					{
						...
						innerItem.add(new ListView<CompoundVoteChoice>("scores", ModelMaker
							.wrap(scores))
						{
							...
							protected void populateItem(ListItem<CompoundVoteChoice> innerInnerItem)
							{
								...
								innerInnerItem.add(new Label("score", new Model<Integer>(choice
									.getScore())));
	...
	{
		String prehash =
			vote.getCaster().getUsername() + vote.getElection().getStart().getTime() + vote.getId();
